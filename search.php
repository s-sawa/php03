...
if (!empty($s_name) || !empty($s_r_name) || !empty($s_r_kinds) || !empty($s_visit_date) || !empty($s_rates)) {
  $sql = "SELECT * FROM my_tabelog WHERE";
  //投稿者検索
  if (!empty($s_name)) {
  ...
  //   }
  //   // if (count($s_r_kinds) == 1) {
  //   //   $search_words[] = ' r_kind = "' . $s_r_kinds[0] . '"';
  //   //   $sql .= ' r_kind = "' . $s_r_kinds[0] . '"';
  //   // } else if (count($s_r_kinds) > 1) {
  //   //   while ($i < count($s_r_kinds)) {
  //   //     $search_words_kinds[] = ' r_kind = "' . $s_r_kinds[$i] . '"';
...
// var_dump(implode(' AND ',$search_words));

//ジャンル
// if (!empty($s_r_kinds)) {
//   $kindCondition = [];
//   foreach ($s_r_kinds as $s_r_kind) {
//     $kindCondition[] = ' r_kind = "' . $s_r_kind . '"';
//   }
//   // $kindCondition = implode(' OR ', $kindCondition);
// }
// var_dump($kindCondition);
